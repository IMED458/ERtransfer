...
export default function App() {
  ...
  // UTC / Real-time clock for top header banner
  ...
  const fetchReferrals = async (showLoading = false) => {
    ...
    try {
      ...
      setError(err.message || "კავშირის შეცდომა სერვერთან");
    } finally {
      setIsLoading(false);
  ...
  // Poll for updates every 5 seconds to support multi-device coordination
  useEffect(() => {
    fetchReferrals(true);
  ...
  const handleAddReferral = async (data: {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errData.error || "მიმართვის დამატება ვერ მოხერხდა");
      }
  ...
  // Action: Update referral status / comment
  const handleUpdateStatus = async (
    ...
    try {
      const response = await fetch(`/api/referrals/${id}`, {
        ...
        body: JSON.stringify({ status, emergencyComment: comment }),
      });
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-12 antialiased">
      {/* Top Professional Clinic Header */}
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-xs bg-slate-50 hover:bg-slate-150 text-slate-700 font-bold px-3 py-1.5 rounded-xl border border-slate-200 transition cursor-pointer"
              >
                <RefreshCw
                ...
                />
                <span className="hidden md:inline font-sans">სინქრონიზაცია</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
        ...
        {error && (
          <div className="bg-rose-50 border border-rose-200 text-rose-800 rounded-2xl p-4 mb-6 flex items-start gap-3 shadow-xs animate-fade-in">
            ...
            <div>
              <p className="text-sm font-bold font-sans">კავშირის პრობლემა სერვერთან</p>
        ...
        <div className="flex p-1 bg-slate-200/70 rounded-2xl max-w-lg mb-6 shadow-2xs">
          <button
            ...
            onClick={() => {
              setActiveTab("doctor");
              // Clear URL search params or do safe UX resets if needed
            }}
        ...
        {isLoading && referrals.length === 0 ? (
          <div className="bg-white border border-slate-100 rounded-3xl py-24 text-center shadow-xs">
            ...
            <p className="text-sm text-slate-500 font-sans">იტვირთება კლინიკური მონაცემები...</p>
          </div>
        ) : (
